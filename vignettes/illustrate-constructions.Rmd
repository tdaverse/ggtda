...
To visualize the 0- and 1-simplices, we use `stat_simplicial_complex()` with `max_dimension = 1L` and `one_simplices = "all".

```{r 1skeleton}
ggplot(unif_df, aes(x = x, y = y)) +
  coord_fixed() +
  stat_simplicial_complex(radius = rad_val,
                          one_simplices = "all", max_dimension = 1L) +
  theme_minimal()
```
...
Since edges themselves do not effectively visualize the entire simplicial complex (users do not want to count edges to determine if a set of 7 points are indeed all pairwise connected), we set `max_dimension = 2L` to add the faces of the Vietoris--Rips complex. This produces the 2-skeleton.
...
ggplot(unif_df, aes(x = x, y = y)) +
  coord_fixed() +
  stat_simplicial_complex(diameter = rad_val * 2,
                          one_simplices = "all", max_dimension = 2L) +
  theme_minimal()
```
...
ggplot(annulus_df, aes(x = x, y = y)) +
  coord_fixed() +
  stat_simplicial_complex(diameter = diam[1], max_dimension = 2L,
                          fill = "darkgoldenrod") +
  theme_minimal()
```
...
ggplot(annulus_df, aes(x = x, y = y)) +
  coord_fixed() +
  stat_simplicial_complex(diameter = diam[2], max_dimension = 2L,
                          fill = "darkgoldenrod") +
  theme_minimal()
```
...
ggplot(annulus_df, aes(x = x, y = y)) +
  coord_fixed() +
  stat_simplicial_complex(diameter = diam[3], max_dimension = 2L,
                          fill = "darkgoldenrod") +
  theme_minimal()
```
